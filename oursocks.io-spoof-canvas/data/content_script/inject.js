...
var inject = function (CONSTANTRAND) {
  var overwrite = function (name) {
    const OLD = HTMLCanvasElement.prototype[name];
    Object.defineProperty(HTMLCanvasElement.prototype, name, {
      "value": function () {
        var shift = {
          'r': Math.floor(CONSTANTRAND * 10) - 5,
          'g': Math.floor(CONSTANTRAND * 10) - 5,
          'b': Math.floor(CONSTANTRAND * 10) - 5,
          'a': Math.floor(CONSTANTRAND * 10) - 5
        };
...
chrome.storage.local.get(['fingerprint_seed'], function(result) {
  var fingerprint_seed = result.fingerprint_seed;

  var script_1 = document.createElement('script');
  if (fingerprint_seed) {
    script_1.textContent = "(" + inject + ")(" + fingerprint_seed + ")";
  }
  else {
    script_1.textContent = "(" + inject + ")(0.65678)";
  }
